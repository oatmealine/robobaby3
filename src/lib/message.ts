import { Channel, GuildMember, Message, Permissions, TextChannel, User } from "discord.js";
import { delay, getRandomEmoji } from "./util";
require("dotenv").config();

export async function sendMessage(location: TextChannel | Message | User, content: string, maxThinkDuration: number = 0): Promise<Message | null> {
...
export const removeInvites = (message: Message): void => {
  ...
  if (message.content.match(/(https?:\/\/)?(www\.)?(discord\.(gg|io|me|li)|discordapp\.com\/invite)\/.+[a-z]/g)) {
    message.delete();
    sendMessage(message.author, `Don't send invite links to other servers. If you must, send them to the interested parties directly. ${getRandomEmoji(message.guild)}`);
  }
};
